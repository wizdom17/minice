...
export default function AdminTransactionsTable({ data }: { data: Trx[] }) {
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await axios.post("/api/admin/delete-transaction", {
        id,
      });
      if (res.status === 200) {
    ...
    // Add logic to handle acceptance of withdrawal
  };
